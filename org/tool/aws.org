...
Really, that's what they basically said as below and it works that way.
...
However this behavior is not consistent as soon as you use this as a ProxyCommand via =ssh=
...
I didn't use =-i /path/my-key-pair.pem= because my ssh tries the key I want to use automatically which might be the case for you too
